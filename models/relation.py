...
class Relation(BaseLearner):
    ...
    def loss_relation(self, student_logits, teacher_logits):
        T = self.args["temperature"]
        Y_s = F.softmax(student_logits/T, dim=1)
        Y_t = F.softmax(teacher_logits/T, dim=1)
        inter_loss = T**2 * self.inter_class_relation(Y_s, Y_t)
        intra_loss = T**2 * self.intra_class_relation(Y_s, Y_t)
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                inputs, targets = inputs.to(self._device), targets.to(self._device)
                logits = self._network(inputs)["logits"]
                old_teacher_logits = self.teacher_model_old(inputs)["logits"]
                new_teacher_logits = self.teacher_model_new(inputs)["logits"]
                ...
                # loss_cls = torch.tensor([0]).to(self._device)
                ...
                old_logits = self.teacher_model_old(inputs)["logits"]
                new_logits = self.teacher_model_new(inputs)["logits"]
                ...
                loss = loss_inter_old + loss_inter_new + loss_inter
                # print("{:.3f}|{:.3f}|{:.3f}|{:.3f}|{:.3f}|".format(loss_cls, loss_inter_new , loss_intra_new , loss_inter , loss_intra))
                print("{:.4f}|{:.4f}|{:.4f}|{:.4f}|{:.4f}|{:.4f}|{:.4f}".format(loss_cls.item(), loss_inter_old.item(), loss_intra_old.item(),\
                                                                                loss_inter_new.item(), loss_intra_new.item(),\
                                                                                loss_inter.item(), loss_intra.item(),
                                                                                ))
